...
import { ListenContent } from '@/types/content';
...
export default function EditAudioPage() {
  ...
  useEffect(() => {
    const checkOwnershipAndLoadContent = async () => {
      ...
      try {
        ...
        if (!contentData || contentData.type !== 'listen') {
          toast({
            title: 'Error',
            description: 'Audio content not found or invalid.',
            variant: 'destructive',
          });
          router.push(`/${handle}`);
          return;
        ...
        const audioContent = contentData as ListenContent;
        setTitle(audioContent.title);
        setPodcastName(audioContent.podcastName || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Success',
        description: 'Your audio content has been updated.',
      });
    ...
    <div className="container mx-auto px-4 pt-24 md:pt-36 pb-24 md:pb-40">
      <div className="max-w-3xl mx-auto">
        ...
        <Card className="bg-black/20 backdrop-blur-md border-white/20">
          <CardHeader>
            <CardTitle className="text-2xl text-white">Edit Audio Content</CardTitle>
            <CardDescription className="text-white/70">
              Update the details of your audio content.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex justify-end pt-4">
                <Button
                  ...
                  disabled={loading}
                >
                  {loading ? 'Updating...' : 'Update Audio Post'}
                </Button>
              </div>
            </form>
          </CardContent>
